...
public class DatabaseHelper extends SQLiteOpenHelper// 继承SQLiteOpenHelper类
{
	...
	public DatabaseHelper(Context context) {
		super(context, Config.DATANAME, null, DATABASE_VERSION);

		// 数据库实际被创建是在getWritableDatabase()或getReadableDatabase()方法调用时
		Log.d(Config.LOG_TAG, "DatabaseHelper Constructor");
	...
	public void onCreate(SQLiteDatabase db) {
		...
		Log.d(Config.LOG_TAG, "DatabaseHelper onCreate");

		// 构建创建表的SQL语句（可以从SQLite Expert工具的DDL粘贴过来加进StringBuffer中）
		StringBuffer sBuffer = new StringBuffer();

		sBuffer.append("CREATE TABLE [" + TABLE_NAME + "] (");
		sBuffer.append("[_id] VARCHAR NOT NULL PRIMARY KEY, ");
		sBuffer.append("[workTicketNumber] VARCHAR,");
		sBuffer.append("[jobName] VARCHAR,");
		sBuffer.append("[startTime] DATETIME,");
		sBuffer.append("[endTime] DATETIME,");
		sBuffer.append("[endMark] BOOLEAN,");
		sBuffer.append("[syncMark] INTEGER,");
		sBuffer.append("[photo] VARCHAR,");
		sBuffer.append("[video] VARCHAR,");
		sBuffer.append("[jobRecord] VARCHAR)");
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		Log.d(Config.LOG_TAG, "DatabaseHelper onUpgrade");
	...
	public void onOpen(SQLiteDatabase db) {
		...
		Log.d(Config.LOG_TAG, "DatabaseHelper onOpen");
	}
